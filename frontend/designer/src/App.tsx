...
import {
  ...
  nextGridField,
  nextScreenId,
  sanitizeCommunicationsForComponents,
  snap,
  stripScreenCommunication,
} from './designerUtils';
import {
  apiStatus,
  deleteCommunicationFormatById,
  deleteMenuById,
  deleteScreenById,
  ...
  isApiError,
  login,
  saveCommunicationFormat as saveCommunicationFormatApi,
  saveCommunicationFormats,
  saveMenu,
  saveScreen,
  setAuthToken,
} from './designerApi';
...
export default function App() {
  ...
  const loadDesignerMetadata = useCallback(async () => {
    const metadata = await fetchDesignerMetadata();
    setScreens([...metadata.screens]);
    setMenus([...metadata.menus]);
    setExpandedMenuKeys((prev) => {
      const menuIds = new Set(metadata.menus.map((menu: MenuSummary) => menu.id));
      const keptKeys = prev.filter((key) => menuIds.has(String(key)));
      return keptKeys.length > 0 ? keptKeys : metadata.menus.map((menu: MenuSummary) => menu.id);
    });
    setCommunicationFormats([...metadata.communicationFormats]);
    setMetadataRevision((prev) => prev + 1);
    setSelectedFormatId((prev) =>
      metadata.communicationFormats.some((format: CommunicationFormat) => format.id === prev)
        ? prev
        : metadata.communicationFormats[0]?.id ?? '',
    );
  }, []);
  ...
  const loadScreen = async (screenId: string, menuOverride?: MenuSummary) => {
    try {
      ...
      const nextComponents = Array.isArray(screen.components) ? screen.components : [];
      const nextCommunications = Array.isArray(screen.communications)
      ...
      const name = screen.name || screenId;
      const linkedMenu = menuOverride ?? menus.find((menu) => menu.screenId === screenId);
      applyFormValues({
        screenId,
        name,
        allowRuntimePersonalization: Boolean(screen.allowRuntimePersonalization),
        isInitialScreen: Boolean(screen.isInitialScreen),
  ...
  const deleteScreen = async (screenId: string) => {
    ...
    try {
      ...
      await deleteScreenById(screenId);
      const menuDeleteResults = await Promise.allSettled(
        linkedMenus.map((menu) => deleteMenuById(menu.id)),
      );
  ...
  const executePreviewAction = async (triggerComponentId: string) => {
    ...
    try {
      const result = await executeCommunication(action.formatId || action.id, input);
      setPreviewGridRows((prev) => {
        const next = { ...prev };
        action.outputBindings.forEach((binding) => {
          const rows = Array.isArray(result[binding.field])
  ...
  const saveCommunicationFormat = async (format: CommunicationFormat) => {
    try {
      await saveCommunicationFormatApi(format);
  ...
  const deleteCommunicationFormat = async (formatId: string) => {
    try {
      await deleteCommunicationFormatById(formatId);
      setCommunicationFormats((prev) => {
  ...
  const saveGridRowsToLinkedFormat = async (componentId: string) => {
    ...
    try {
      await saveCommunicationFormatApi(nextFormat);
  ...
  const saveCurrentScreen = async (
    ...
    await saveCommunicationFormats(communicationFormats);
  ...
  const saveScreenSettings = async () => {
    ...
    try {
      ...
      if (previousScreenId && previousScreenId !== values.screenId) {
        await deleteScreenById(previousScreenId);
      }
      applyFormValues(values);
      setScreens((prev) => [
        ...prev.filter(
  ...
  const onLogin = async (values: { username: string; password: string }) => {
    try {
      ...
      const nextToken = loginResult.token;
